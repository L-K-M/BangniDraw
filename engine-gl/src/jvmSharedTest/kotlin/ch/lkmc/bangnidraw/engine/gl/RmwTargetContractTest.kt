...
class RmwTargetContractTest {
    ...
    fun `pressure-sized RMW targets retain sufficient capacity`() {
        val source = File(repositoryRoot(), SMUDGE_PASS_PATH).readText().withoutWhitespace()
    ...
    fun `offscreen targets separate logical size from allocated capacity`() {
        val source = File(repositoryRoot(), OFFSCREEN_TARGET_PATH).readText().withoutWhitespace()

        assertTrue("valcapacityWidth:" in source)
        assertTrue("valcapacityHeight:" in source)
    ...
    private companion object {
        ...
        const val SMUDGE_PASS_PATH =
            "engine-gl/src/jvmShared/kotlin/ch/lkmc/bangnidraw/engine/gl/SmudgePass.kt"
        const val OFFSCREEN_TARGET_PATH =
            "engine-gl/src/jvmShared/kotlin/ch/lkmc/bangnidraw/engine/gl/OffscreenTarget.kt"
    }
}

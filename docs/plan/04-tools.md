...
| --- | --- | --- | --- |
| Pencil, Charcoal, Ink pen, Paintbrush, Airbrush, Marker | `Brush(preset)` | `DabPass` → stroke buffer → merge on pen-up | active layer |
...
| Charcoal | 28 (4–180) | 0.85 | 0.18 | 0.7 | 0.15 | Round / Fixed | 0.15 | off | – |
...
| Charcoal | `Curve(0.55, 0.68, 0.82, 1)` | `Curve(0.1, 0.35, 0.7, 1)` | `Curve(0.05, 0.25, 0.6, 1)` | size 2.8 at flat, opacity 0.55, elongate | none | size 0.25, pos 0.25 | Accumulate |
...
- **Charcoal.** Low flow and paper-anchored grain make a broken carbon mark
  that builds under repeated passes. Pressure darkens it; tilt broadens and
  elongates it for side shading, while jitter keeps the edge irregular.

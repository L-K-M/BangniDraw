...
| Foldable / rotation surface resize mid-stroke | viewport-sized targets are wrong size | `surfaceChanged` **finishes** (commits) the live stroke first — the stroke buffer is canvas-space, so the merge is valid and no work is lost (`03-canvas-engine.md` §8.6) — then reallocates `Accum`/`Scratch`, rebases the view and re-composites. Only `surfaceDestroyed` *cancels* (there is no target left to commit into, `02-architecture.md` §8.2). Checklist row U1 in `11-testing.md` §8. |

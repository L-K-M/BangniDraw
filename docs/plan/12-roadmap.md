...
| Resources | `values/strings.xml`, `values-b+zh+Hans/strings.xml`, adaptive icon from `media-sources/icon.png` via `scripts/generate_icons.py` (full-bleed foreground PNGs per density over solid indigo; corrected after the v1.0.0 device report) plus the hand-authored monochrome brush vector `drawable/ic_launcher_monochrome.xml`, `xml/backup_rules.xml` + `data_extraction_rules.xml` excluding `projects/` |
...
copies and color mixing; the debug build also exposes its latency overlay.
About shows the version, autosave/storage behavior, Unlicense status and the
linked Mixbox notice. English and zh-Hans resources have matching translated
keys, the README states the release-candidate boundary, and `CHANGELOG.md`
...
### Step 13 — Selectable application themes (S, **landed on main** — PRs #141 and #148, 2026-08-28, CI green; the device check has not been run — no device has ever been available)

...
| `BrushPreset`, `BrushModel`, `ToolKind` | The JSON-serializable brush description, its footprint model (`Standard` / `ChineseInk`), and the sealed interface `Brush(preset)`, `Smudge`, `Water`, `Blur`, `Fill`, `Eyedropper` (docs/plan/04-tools.md §1 — erasers are `Brush` presets with `eraseMode`, not a kind). |

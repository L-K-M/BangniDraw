...
class CanvasRenderer(
    ...
    fun refreshWatercolorOverlay(): WatercolorOverlayKernel.RefreshResult {
        val current = stack
    ...
    fun hasWatercolorOverlay(): Boolean =
    ...
    fun onContextCreated(strict: Boolean): Boolean {
        ...
        caps = probed
        GlLog.i(GL_TAG, "GL context: ${probed.describe()}")
        if (!probed.isSupported) {
            GlLog.w(GL_TAG, "unsupported device: ${probed.describe()}")
            isReady = false
            return false
        }
        ...
        try {
            ...
            GlLog.e(GL_TAG, "engine startup failed on ${probed.describe()}", e)
            isReady = false
            return false
        }
    ...
    val strokePreviewDirty: IntRect

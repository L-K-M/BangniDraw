...
class SaveableTransientContractTest {
    ...
    fun `studio card dialogs and their drafts are saveable`() {
        val studio = ContractTestSources.read(STUDIO_PATH)
        ...
        assertTrue(
            "var text by rememberSaveable { mutableStateOf(painting.title.orEmpty()) }" in studio,
    ...
    fun `help popups are saveable`() {
        val info = ContractTestSources.read(INFO_HELP_PATH)
        assertTrue(
        ...
        val strip = ContractTestSources.read(TOP_STRIP_PATH)
        assertTrue(

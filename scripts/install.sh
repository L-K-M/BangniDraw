#!/usr/bin/env bash
#
# Build 帮你Draw and install it on a connected Android device, then launch it.
#
# Handy when working from IntelliJ IDEA (which lacks Android Studio's Run
# button): connect the tablet with USB debugging enabled (or pair over Wi-Fi
# with `adb connect <ip>:<port>`) and run this from the IDE terminal.
#
# Usage:
#   ./scripts/install.sh             build + install + launch the debug build
#   ./scripts/install.sh --release   build + install the release build
...
case "$VARIANT" in
  debug)   TASK="installDebug";   APP_ID="ch.lkmc.bangnidraw.debug" ;;
  release) TASK="installRelease"; APP_ID="ch.lkmc.bangnidraw" ;;
esac

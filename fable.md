...
Nine PRs, each on its own branch, each reviewed round by round under
`CLAUDE.md`'s rules and merged at steady state. The review record —
every finding applied, declined or refuted, with its evidence — is in
`REVIEW.md` under `R-185` – `R-238`.

| PR | Item | What landed |
| --- | --- | --- |
| [#175](https://github.com/L-K-M/BangniDraw/pull/175) | F-1, F-2 | A failed Gallery write stays hidden and retryable instead of publishing a truncated image; a pending row stranded by process death is reclaimed rather than abandoned; `sync` contains provider faults — including the `RemoteException` family, which is a *sibling* of `RuntimeException`, not a subtype — at all three of its cross-process calls. Nine rounds. |
| [#176](https://github.com/L-K-M/BangniDraw/pull/176) | F-7 | `DabPass` orphans its instance VBO before uploading, matching the rule `CompositePass` already documented. Four rounds. |
| [#177](https://github.com/L-K-M/BangniDraw/pull/177) | — | The five contract-test fixes ANALYSIS had deferred, plus `ContractTestSources.readNormalized`/`readCompact`, so a mechanical reformat can no longer false-fail a pin. Three rounds. |
| [#178](https://github.com/L-K-M/BangniDraw/pull/178) | F-9, F-11 | Tool-sheet sliders publish once, on release, instead of republishing the whole Canvas `UiState` per pointer sample; the layer row's blend-mode caption ellipsizes instead of clipping mid-glyph. Five reviewed rounds, then one the reviewer never returned — see below. |
| [#179](https://github.com/L-K-M/BangniDraw/pull/179) | F-3 | Every atomic write gets its own scratch file, so concurrent writers cannot truncate each other; `sweepTmp` spares live writers; `BrushPresetStore` is serialized like its sibling. Three rounds. |
| [#180](https://github.com/L-K-M/BangniDraw/pull/180) | F-17 | CI builds the configuration that actually ships (Mixbox in, R8 on), not only debug and the stripped release; `.gitignore` no longer swallows source directories named `build`. Six rounds. |
| [#181](https://github.com/L-K-M/BangniDraw/pull/181) | F-13–F-16 | One term per thing in the Chinese strings, with the terminology and the label-to-help relationship both pinned. Nine reviewed rounds, then one the reviewer never returned. |
| [#182](https://github.com/L-K-M/BangniDraw/pull/182) | F-4 | Merging down warns before it clears the surviving layer's alpha lock, and the dialog names what the merge would actually change. Two rounds. |
| [#183](https://github.com/L-K-M/BangniDraw/pull/183) | F-8 | `PointerSample.tool` is filled only where it is read, so a move, a lift, a hover move and every predicted sample stop paying a `getToolType` JNI call whose answer is discarded; the field is nullable, so a future reader cannot get another pointer's tool. Two reviewed rounds, then one the reviewer never returned. |

**Not implemented, and why.** **F-5** (Fill's "expand" reaching about twice its
setting around a corner) is a behavior change that needs a product call and a
4096² measurement. **F-18** (the desktop `GLES30` actual handing LWJGL heap
buffers) cannot be verified without a desktop GL context, and an unverifiable
rewrite of twelve GL entry points is worse than a precise record. Everything
else above that was not implemented — F-6, F-10, F-12, F-19 through F-25 — is
carried into `ANALYSIS.md` under a stable id, stated fully enough to pick up
cold.

**Three PRs merged without a review of their last round**, and the reviewer's
failure mode changed over the session. Early rounds returned in six minutes; by
the end, seven attempts across three PRs sat in the Z.ai step for 15 to 36
minutes and posted nothing at all. The last two — #181's and #183's — were on
`main`-merge heads, the largest diff the reviewer sees, and #178's reports
carried five rotating unchanged sections, the largest scope of the set. So the
failures track diff size rather than a reviewer that is simply down.

`CLAUDE.md` says an unreviewed round after its re-run must not stall the
pipeline. For each: android was green on the merged head, and the diff was
re-read adversarially in place of the review. The merge commits say which round
went unreviewed and what that read covered — narrow, for #181 and #183, because
the substance had been reviewed through rounds 9 and 2 respectively and the
unreviewed delta was one conflict resolution and one test-needle widening, each
verified empirically rather than by reading.

That read is not a formality. On #183 it caught that round 1's Major had named
three places carrying a claim the code no longer supported, and only two had
been fixed. On #178 it found nothing wrong with the code — but the merge
commit's own account of the hang was wrong, saying 40 minutes where the third
attempt ran about 15, and
[the correction is a comment on the PR](https://github.com/L-K-M/BangniDraw/pull/178#issuecomment-5478312579).
All of it is recorded here for one reason: an unreviewed merge is fine to do
and bad to hide, and so is a wrong number in the record that justifies one.

...
| --- | --- | --- | --- |
| Pencil, Ink pen, Paintbrush, Dry brush, Airbrush, Marker | `Brush(preset)` | `DabPass` → stroke buffer → merge on pen-up | active layer |
...
- **Paintbrush.** Flat(0.7) tip following stroke direction gives a
  ...
  early).
- **Dry brush.** A narrow, path-oriented tip combines low flow with
  paper-anchored grain and jitter. Pressure fills the broken mark gradually;
  pigment mixing remains available without making one pass look wet.
